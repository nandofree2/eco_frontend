...
export const api = {
  ...
  invoices: {
    list: async (query?: string, sort?: string, page: number = 1, perPage: number = 10, branchId?: string, customerName?: string,
      ...
      if (invoicedDateFrom) params.append('q[delivery_order_invoiced_date_gteq]', invoicedDateFrom);
      if (invoicedDateTo) params.append('q[delivery_order_invoiced_date_lteq]', invoicedDateTo);
      if (sort) params.append('q[s]', sort);
      params.append('page', page.toString());
      params.append('per_page', perPage.toString());
  ...
  customer_statements: {
    ...
    print: async (query?: string, contact_name?: string, sort?: string, page: number = 1, perPage: number = 20, transactionDateFrom?: string, transactionDateTo?: string): Promise<any> => {
      const params = new URLSearchParams();
      if (query) params.append('q[code_or_description_cont]', query);
      if (contact_name) params.append('q[contact_name_cont]', contact_name);
      if (sort) params.append('q[s]', sort);
