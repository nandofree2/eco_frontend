    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden transform transition-all border border-gray-100">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className={`block text-sm font-bold mb-1 ${hasError('code') ? 'text-red-600' : 'text-gray-700'}`}>SKU Code</label>
            <input
              ref={codeRef}
              type="text"
              required
            ...
            />
            {hasError('code') && (
              <p className="mt-1 text-xs text-red-500 font-bold">SKU {serverErrors?.code?.join(', ')}</p>
            )}
          </div>
