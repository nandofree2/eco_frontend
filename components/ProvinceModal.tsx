...
interface ProvinceModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: Partial<Province>) => Promise<void>;
  province?: Province | null;
  loading: boolean;
  serverErrors?: Record<string, string[]> | null;
}

const ProvinceModal: React.FC<ProvinceModalProps> = ({ isOpen, onClose, onSubmit, province, loading, serverErrors }) => {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden transform transition-all border border-gray-100">
        <div className="bg-eco-600 px-6 py-4 flex items-center justify-between">
          <h2 className="text-xl font-bold text-white flex items-center gap-2">
            ...
            {province ? 'Edit Province' : 'New Province'}
          </h2>
          <button 
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className={`block text-sm font-bold mb-1 ${hasError('name') ? 'text-red-600' : 'text-gray-700'}`}>Province Name</label>
            <input
              ref={nameRef}
              type="text"
              required
          ...
          <div>
            <label className={`block text-sm font-bold mb-1 ${hasError('code') ? 'text-red-600' : 'text-gray-700'}`}>Province Code</label>
            <input
              ref={codeRef}
              type="text"
              required
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Provide context for this province..."
              rows={3}
          ...
          <div className="pt-4 flex gap-3">
            ...
            >
              ...
              {province ? 'Update Province' : 'Create Province'}
            </button>
          </div>
        </form>
      </div>
